...
- 多个存储卷的频道合并为一个频道序列；
- 切换到另一个存储卷的频道与普通切台行为一致；
- 当前存储被拔出时，默认进入“设备已移除”状态，不自动跳到另一 U 盘，以便用户重新插回后原位恢复；
- 若应用启动时上次频道离线，但存在其他 Visible Channel，则自动播放频道号最小的在线频道；
- 在其他频道已正常播放时，插入新的 U 盘不得强制切台。

实现跟踪（2026-08-04）：当前版本只保存一个 U 盘根 URI，尚未完成多个卷的
挂载监听、选择和自动恢复。后续实现前必须先将卷身份与访问 URI 分离：卷身份优先使用
规范化的文件系统 UUID / MediaStore volume name，不得使用可重复的卷标、容量或设备规格，
也不得直接哈希包含临时挂载路径的完整 URI。访问 URI、显示卷标和容量只作为当前挂载信息。
需要覆盖 A 盘换 B 盘、同名卷标、同时插入多个盘、重新插回原盘、重新格式化以及厂商不暴露
UUID 的降级行为，并确认最终采用“合并所有在线频道”还是“单一当前盘的换碟模式”。
...
- 当前播放不得中断；
- 对应频道从 Visible Channel 集合移除；
- 历史保留；
- 频道列表在下一次打开或当前快照提交后更新。
...
DataStore 只保存第 14 章的全局设置和少量 UI 偏好。频道历史与媒体索引不得拆散存入多个无事务保证的 Preferences key。
...
**Given** 两个 U 盘均已授权且包含有效频道
**When** 同时在线
**Then** 所有频道在同一频道序列中显示，频道号唯一，历史按 `volumeId + path` 独立保存
