...
系统自动挂载的可移除卷优先读取 MediaStore 元数据，不得同时再对同一卷执行完整文件树遍历。
Android 在刚挂载时可能只返回尚未完成的媒体库结果，因此这类结果必须按渐进快照处理：新频道和
Episode 可以立即合并显示，但不得用较小的临时结果隐藏上一份有效索引。应用应依据 MediaStore
generation/version 退避观察；版本未变化时不得重复查询和写入 Room。收到系统媒体扫描完成事件或
用户明确执行“重建索引”后，才可将结果作为可移除缺失条目的权威快照。
