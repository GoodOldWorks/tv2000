...
自动挂载路径还必须验证有界直接目录快照：冷启动应直接得到完整频道；已有部分 Room 索引时先恢复
播放，再在后台用目录快照替换为完整结果；挂载尚未就绪时允许有限重试，成功后必须停止。应用重启且
MediaStore 版本未变化时不得再次遍历文件树，节目 URI 应保持为可读的稳定文件路径。
...
慢盘可靠性测试应分别记录目录快照、系统 MediaStore 索引和 TV2000 查询。除暂时失败后的有限重试外，
同一 MediaStore generation/version 不得重复遍历文件树、查询媒体库或写 Room。
